...
- [ ] 🟡 Jernej: pregled mojih datotek (`useParentLinks`, `useBatchSetup`, `useMetadataForm`, `useProcessing`, `stores/useMetadata`, `stores/useUpload`, `domain/metadata-wire` — kam spada?); ~~odstranitev zastarelega (mojibake/mangled helperji, `doOCR`, `/extract`)~~ **preverjeno 10. 9. 2026 — ni zastarelo, ne brisati**: `repairMangledText` je živ v `services/upload.ts` in pokrit s testi, `doOCR: false` je namerno na vsakem uploadu (OCR teče lokalno). Edini neklicani je `reextractFile`, ki pa ostane — je dokumentirana recovery pot za 201 iz `dto.ts`. Podrobnosti: `docs/REFACTOR.md` §Kaj ni slop; lepši prikaz novega 400 za `extractedTexts` ključe; `?fields=` allowlist v search projekcijah; 14 padajočih `schema.test.ts` (Node ≥24 `localStorage` global — test setup rabi stub); text-only re-upload brez quality warninga
