...
| [Nested record folders & manual selection](nested-record-folders-and-manual-selection.md) | **Arch** (`.rs`) + **GUI** (`.vue`) | `core::fs::scan_root` is one level deep only — a real archive folder that wraps two+ distinct books under one collection folder (`arh/Cèrnagora/` holding two separate 392- and 132-page books) is invisible past the wrapper, and no structural heuristic can tell a "wrapper to descend into" from a "record with an unrelated subfolder to ignore" (two real `arh/` folders have identical shape, opposite correct answer). **Status: done** (shipped 2026-08-27) — the scanner now recurses to any depth and lists every folder as a candidate; a per-row Hide + "Show hidden" toggle replaces any auto-heuristic. |
