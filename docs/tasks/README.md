...
| [Nested record folders & manual selection](nested-record-folders-and-manual-selection.md) | **Arch** (`.rs`) + **GUI** (`.vue`), decision made | `core::fs::scan_root` is one level deep only — a real archive folder that wraps two+ distinct books under one collection folder (`arh/Cèrnagora/` holding two separate 392- and 132-page books) is invisible past the wrapper, and no structural heuristic can tell a "wrapper to descend into" from a "record with an unrelated subfolder to ignore" (two real `arh/` folders have identical shape, opposite correct answer). Decision: no auto-heuristic — the operator browses and picks, like a file explorer. |
